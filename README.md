...
| `ADMIN_EMAIL` | Required for server-side admin password login | Admin email address. Configure in `.env.local` locally and hosting secrets in production. |
| `ADMIN_PASSWORD_HASH` | Required for server-side admin password login | Generate locally with `ADMIN_PASSWORD_PLAINTEXT` and `corepack pnpm auth:hash-password`. Store the hash only. |
...
| `PUBLIC_SITE_URL` | Required for production payments | Public site origin used in BTCPay invoice return links, for example `https://helvetic-reserve.com`. |
...
| `BTCPAY_ENABLED` | Optional until payments launch | Set `1` to create BTCPay checkout invoices for crypto purchase requests. |
| `BTCPAY_SERVER_URL` | Required when `BTCPAY_ENABLED=1` | Your self-hosted BTCPay Server URL. Use HTTPS in production. |
| `BTCPAY_STORE_ID` | Required when `BTCPAY_ENABLED=1` | BTCPay store identifier from the BTCPay store settings/API page. |
| `BTCPAY_API_KEY` | Required secret when `BTCPAY_ENABLED=1` | BTCPay API key with invoice creation/view permissions for the store. Store in `.env.local` or production secrets only. |
| `BTCPAY_WEBHOOK_SECRET` | Required for payment status updates | Secret generated by the BTCPay store webhook. Store in secrets only. |
| `BTCPAY_DEFAULT_CURRENCY` | Optional | Defaults to `CHF`. Used when a watch has no currency. |
...
For real server-side admin login, configure an email and password hash instead of the preview bypass:

```powershell
$env:ADMIN_PASSWORD_PLAINTEXT="<your-admin-password>"
corepack pnpm auth:hash-password
Remove-Item Env:\ADMIN_PASSWORD_PLAINTEXT
```

Put the generated hash in `.env.local`:
...
In production, configure `ADMIN_EMAIL`, `ADMIN_PASSWORD_HASH`, and `JWT_SECRET` in the host's secret manager. Do not commit them.
...
The site integrates with BTCPay Server as the free/self-hosted payment processor. BTCPay creates invoices through its Greenfield API, and the app updates local request payment status only from signed BTCPay webhooks.

Recommended production setup:

1. Deploy BTCPay Server on a VPS or existing host.
2. Create a BTCPay store and enable the coins you want customers to pay with.
3. For XMR, install/configure the BTCPay Monero plugin and its Monero wallet/node requirements.
4. Create a BTCPay API key with the minimum invoice permissions needed to create/view invoices.
5. Create a store webhook pointing to `https://helvetic-reserve.com/api/payments/btcpay/webhook`.
6. Configure these secrets in your host secret manager:

```env
PUBLIC_SITE_URL=https://helvetic-reserve.com
BTCPAY_ENABLED=1
BTCPAY_SERVER_URL=https://pay.helvetic-reserve.com
BTCPAY_STORE_ID=
BTCPAY_API_KEY=
BTCPAY_WEBHOOK_SECRET=
BTCPAY_DEFAULT_CURRENCY=CHF
```

Customers can choose a preferred crypto, including XMR, on the request form. The actual checkout coins are controlled by the BTCPay store. If BTCPay is disabled or a watch has no public price, the request still saves and Helvetic Reserve follows up manually.

BTCPay references:

- BTCPay Greenfield API: https://docs.btcpayserver.org/Development/GreenFieldExample-NodeJS/
- BTCPay invoice metadata: https://docs.btcpayserver.org/Development/InvoiceMetadata/
- BTCPay altcoin/XMR notes: https://docs.btcpayserver.org/FAQ/Altcoin/
- Monero plugin: https://github.com/btcpay-monero/btcpayserver-monero-plugin
